...
/**
 * Filtra un catalogo de empresas a las que la sesion puede ver.
 *
 * Basta con tener acceso a un area de la empresa para que su tarjeta
 * aparezca: la portada es un menu, y esconder la empresa a alguien que si
 * puede ver una de sus areas lo dejaria sin ruta de entrada.
 */
export function empresasVisibles<T extends { id: string }>(sesion: Sesion, catalogo: T[]): T[] {
  if (!sesion.perfil || !sesion.perfil.activo) return []
  if (sesion.perfil.es_direccion) return catalogo
  return catalogo.filter((e) => sesion.permisos.some((p) => p.empresa === e.id))
}
